...
_logger.debug("DLL 路径: %s", _DLL_PATH)
_logger.debug("DLL 存在: %s", _DLL_PATH.exists())
_logger.debug("当前工作目录: %s", os.getcwd())
...
if os.name == "nt":
    ...
    if os.path.isdir(mingw_bin) and mingw_bin not in os.environ["PATH"]:
        _logger.debug("添加 mingw64 路径到 PATH: %s", mingw_bin)
        os.environ["PATH"] = mingw_bin + ";" + os.environ["PATH"]

    # 也将当前 DLL 目录添加到 PATH
    dll_dir = str(_BASE)
    if dll_dir not in os.environ["PATH"]:
        _logger.debug("添加 DLL 目录到 PATH: %s", dll_dir)
        os.environ["PATH"] = dll_dir + ";" + os.environ["PATH"]

# 尝试直接使用绝对路径加载
_dll_path_str = str(_DLL_PATH)
_logger.debug("尝试加载 DLL: %s", _dll_path_str)

# 在 Windows 上尝试使用 WinDLL 而不是 cdll
if os.name == "nt":
    try:
        _logger.debug("尝试使用 WinDLL 加载")
        _dll = C.WinDLL(_dll_path_str, winmode=0)
    except Exception as e:
        _logger.debug("WinDLL 加载失败: %s", e)
        _logger.debug("回退到 cdll 加载")
