...
10    YLG=XLG+69.8    
      ...
      END
C--------------------------------------------------------------------
      SUBROUTINE MSIS86_SDR(PATH, PATH_LEN)
C     Set the data root directory for MSIS86.DAT lookup.
C     Called from the C ABI shim before first GTS5 call.
      INTEGER PATH_LEN
      CHARACTER PATH(*)
      CHARACTER*512 DRROOT
      COMMON/UINR/IUMSIS,MONITO,IIEE
      COMMON/MSIS86DR/DRROOT
      IUMSIS=12
      MONITO=6
      DRROOT=' '
      IF(PATH_LEN.GT.0) THEN
        DO 10 I=1,MIN(PATH_LEN,512)
          DRROOT(I:I)=PATH(I)
   10   CONTINUE
      ENDIF
      END             
