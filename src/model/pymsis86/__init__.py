...
class Model:
    """MSIS-86 / CIRA-86 热层模型 ctypes 封装。

    计算 85 km 以上中性大气温度和物种数密度。
    """
    ...
    def __init__(
        ...
        *,
        ...
        if dll_path is None:
            ...
            if os.name == "nt":
                candidates = (base / "build" / "msis86.dll", base / "msis86.dll")
            else:
                candidates = (base / "build" / "libmsis86.so", base / "libmsis86.so")
            dll_path = next(
        ...
        # 绑定 set_data_root
        self._set_data_root = self._dll.msis86_set_data_root
        self._set_data_root.restype = None
        self._set_data_root.argtypes = [ctypes.c_char_p, ctypes.c_int]

        # 绑定 gts5_eval
        self._gts5_eval = self._dll.gts5_eval
        self._gts5_eval.restype = None
        self._gts5_eval.argtypes = _ARGTYPES

        # 设置数据目录（包含 msis86data 子目录）
        data_root_str = str(self._data_root / "msis86data")
        self._set_data_root(data_root_str.encode("utf-8"), len(data_root_str))
