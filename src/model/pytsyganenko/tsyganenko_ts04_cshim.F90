! tsyganenko_ts04_cshim.F90 — C ABI shim for TS04 magnetic field model
!
!  T04_s uses IMPLICIT REAL*8 internally, but its formal arguments
!  (PARMOD, PS, X, Y, Z, BX, BY, BZ) are explicitly declared REAL
!  (single precision).  The shim converts c_double <-> REAL.
!
module tsyganenko_ts04_cshim
...
contains
  ...
  subroutine tsyganenko_eval(iopt, parmod, ps, x, y, z, bx, by, bz) &
    ...
    call T04_s(iopt, pm_s, real(ps), real(x), real(y), real(z), &
               bx_s, by_s, bz_s)
    bx = dble(bx_s)
    by = dble(by_s)
    bz = dble(bz_s)
...
end module tsyganenko_ts04_cshim
